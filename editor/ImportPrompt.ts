...
export class ImportPrompt implements Prompt {
	...
	public readonly container: HTMLDivElement = div({class: "prompt noSelection", style: "width: 300px;"},
		...
		p({style: "text-align: left; margin: 0.5em 0;"},
			"MusBox songs can be exported and re-imported as .json files. You could also use other means to make .json files for MusBox as long as they follow the same structure.",
		),
		p({style: "text-align: left; margin: 0.5em 0;"},
			"MusBox can also (crudely) import .mid files. There are many tools available for creating .mid files. Shorter and simpler songs are more likely to work well.",
		),
		this._fileInput,
		this._cancelButton,
	);
	...
	private _parseMidiFile(buffer: ArrayBuffer): void {
						...
						case MidiEventType.metaAndSysex: {
							if (eventStatus == MidiEventType.meta) {
								...
								if (message == MidiMetaEventMessage.endOfTrack) {
									...
									// beats. MusBox doesn't support fractional beats in a bar.
		...
		// Now the MIDI file is fully parsed. Next, constuct MusBox channels out of the data.
		...
		// Convert each midi channel into a MusBox channel.
						...
						for (let bar: number = startBar; bar < endBar; bar++) {
							...
							if (noteStartPart < noteEndPart) {
								...
								if (currentBar != bar || pattern == null) {
									...
									// program in this channel, create a new MusBox instrument for it.
								...
								const initialMusBoxPitch: number = Math.round((shiftedHeldPitch + currentMidiInterval) / intervalScale);
								...
								const potentialPins: PotentialPin[] = [
									{part: 0, pitch: initialMusBoxPitch, size: firstPin.size, keyPitch: false, keySize: false}
								];
								...
								for (let part: number = noteStartPart + 1; part <= noteEndPart; part++) {
									...
									// MusBox can only add pins at whole number intervals and sizes. Detect places where
									...
									if (keyPitch || keySize || lastPart) {
										...
										if (addPin) {
											...
											note.pins.push(makeNotePin(toBePinned.pitch - initialMusBoxPitch, toBePinned.part, toBePinned.size));
								...
								note.pins.push(makeNotePin(lastToBePinned.pitch - initialMusBoxPitch, lastToBePinned.part, lastToBePinned.size));
								...
								// Build the note chord out of the current pitches, shifted into MusBox channelBasePitch relative values.
								note.pitches.length = 0;
		...
		// For better or for worse, MusBox has a more limited number of channels than Midi.

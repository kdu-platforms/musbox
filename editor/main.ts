...
const musboxEditorContainer: HTMLElement = document.getElementById("musboxEditorContainer")!;
musboxEditorContainer.appendChild(editor.mainLayer);
editor.whenUpdated();
...
// MusBox uses browser history state as its own undo history. Browsers typically
// remember scroll position for each history state, but MusBox users would prefer not
...
// When compiling synth.ts as a standalone module named "musbox", expose these classes as members to JavaScript:

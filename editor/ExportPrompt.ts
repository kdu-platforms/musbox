...
export class ExportPrompt implements Prompt {
	private readonly _fileName: HTMLInputElement = input({type: "text", style: "width: 10em;", value: "MusBox-Song", maxlength: 250, "autofocus": "autofocus"});
	...
	private readonly _formatSelect: HTMLSelectElement = select({style: "width: 100%;"},
		...
		option({value: "json"}, ".json (for any MusBox version)"),
		option({value: "html"}, ".html (opens MusBox)"),
	);
	...
	private _exportToMidi(): void {
		...
		const midiTicksPerMusBoxTick: number = 2;
		const midiTicksPerBeat: number = midiTicksPerMusBoxTick * Config.ticksPerPart * Config.partsPerBeat;
		const midiTicksPerPart: number = midiTicksPerMusBoxTick * Config.ticksPerPart;
		...
		for (const track of tracks) {
			...
			if (isMeta) {
				...
				writer.writeMidiAscii("Composed with https://kdu-platforms.github.io/musbox-web");
					...
					if (prevInstrumentIndex != instrumentIndex) {
						...
						if (!isDrumset) {
							...
							if (preset != null && preset.midiProgram != undefined) {
								...
								// The first MusBox drumset channel is handled as a Midi drumset channel and doesn't need a program, but any subsequent drumsets will just be set to taiko.
				...
				for (const bar of unrolledBars) {
					...
					if (pattern != null) {
						...
						for (let noteIndex: number = 0; noteIndex < pattern.notes.length; noteIndex++) {
							...
							// This may involve offsetting this base pitch from MusBox's note pitch.

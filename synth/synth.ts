...
export class FilterSettings {
	...
	public convertLegacySettings(legacyCutoffSetting: number, legacyResonanceSetting: number, legacyEnv: Envelope): void {
		...
		if (legacyEnv.type == EnvelopeType.none && !resonant && cutoffAtMax) {
			...
			// However, MusBox's original lowpass filters behaved oddly as they
...
// Settings that were available to old versions of MusBox but are no longer available in the
...
export class Song {
	private static readonly _format: string = "MusBox";
	...
	public fromBase64String(compressed: string): void {
		...
		if (beforeNine) {
			// Unfortunately, old versions of MusBox had a variety of different ways of saving
		...
		while (charIndex < compressed.length) switch(command = compressed.charCodeAt(charIndex++)) {
			...
			case SongTagCode.effects: {
				...
				if (beforeNine) {
					...
					// MusBox currently uses two base64 characters at 6 bits each for a bitfield representing all the enabled effects.
...
// When compiling synth.ts as a standalone module named "musbox", expose these imported classes as members to JavaScript:
